...
int search(int rp, int j, int s0, int s1, int sign, struct _seg *answer) {
  ...
  for (n = next_up(j); n != next_up(rp); n = next_up(n)) {
    if (0 && debug)  uWarn("d_ring[%d].s.ss[%d]=%f d_ring[%d].s.ss[%d]=%f",
    ...
    if (0 && debug)  uWarn("slope %d%d%d [%d,%d] = %f",
...
double find_shift(double slope, double offset) {
  ...
  if (debug) { uWarn("find_shift %f %f -> %f", slope, offset, shift); }
...
void polygon_point(struct _seg *s) {
  double l, r;
  if (debug) { uWarn("loop %f %f", s->slope, s->offset); }
  l = find_shift(- s->slope,   s->offset);
  r = find_shift(s->slope, - s->offset);
  if (l < df.l_min) { df.l_min = l; }
  if (r < df.r_min) { df.r_min = r; }
  if (debug) { uWarn("constraint left:  %f %f", l, df.l_min); }
  if (debug) { uWarn("constraint right: %f %f", r, df.r_min); }
}
...
double find_df_center(struct _seg *min, struct _seg *max, double gross_df) {
  ...
  if (debug) { uWarn("find_df_center %f %f", delta, factor); }
  return factor * delta;
}
...
int contemplate_data(unsigned int absolute, double skew, double errorbar, int freq) {
  ...
  if (debug) { uWarn("xontemplate %u %.1f %.1f %d", absolute, skew, errorbar, freq); }
  ...
  if (valid == RING_SIZE) {
    ...
    for (j = rp; ; j = n) {
      ...
      if (debug)  uWarn("hist %d %d %f %f %f", j, d_ring[j].absolute - absolute,
    ...
    if (!inconsistent) {
      ...
      if (dinit && debug) { uWarn("found maxseg vs. save_min inconsistency"); }
      ...
      if (dinit && debug) { uWarn("found minseg vs. save_max inconsistency"); }
      if (dinit) { inconsistent = 1; }
      min_imax = c;
    ...
    if (inconsistent) {
      ...
      if (debug) { uWarn("find_df() = %e", delta_f); }
      delta_f += find_df_center(&save_min, &save_max, delta_f);
      delta_freq = delta_f * 65536 + .5;
      if (debug) { uWarn("delta_f %f  delta_freq %d  bsn %d", delta_f, delta_freq, both_sides_now); }
